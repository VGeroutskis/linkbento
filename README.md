...
- **11 Themes** — Dark, Light, Cyberpunk, Sunset, Ocean, Forest, Neon, Midnight, Rose, Aurora + Auto (system preference)
- **Bilingual** — Greek (EL) / English (EN) with full i18n support
- **Fully Dynamic** — Everything is configured from a single `CONFIG` object in `script.js`
...
All settings are in [`script.js`](script.js). Just edit the `CONFIG` object and the `translations` object:
...
1. Add a new CSS variable block in [`styles.css`](styles.css)
2. Add the theme option in [`index.html`](index.html) inside `.theme-options`
3. The theme system handles the rest automatically

### 📁 Project Structure

```
├── index.html       # Page structure (no hardcoded personal data)
├── script.js        # CONFIG, translations, all logic & animations
├── styles.css       # Themes, layout, responsive styles
...
- **11 Θέματα** — Dark, Light, Cyberpunk, Sunset, Ocean, Forest, Neon, Midnight, Rose, Aurora + Auto (ακολουθεί το σύστημα)
- **Δίγλωσσο** — Ελληνικά (EL) / Αγγλικά (EN) με πλήρες i18n
- **Πλήρως Δυναμικό** — Όλα ρυθμίζονται από ένα `CONFIG` object στο `script.js`
...
Όλες οι ρυθμίσεις βρίσκονται στο [`script.js`](script.js). Επεξεργάσου μόνο το `CONFIG` object και το `translations` object:
...
1. Πρόσθεσε νέο CSS variable block στο [`styles.css`](styles.css)
2. Πρόσθεσε την επιλογή θέματος στο [`index.html`](index.html) μέσα στο `.theme-options`
3. Το σύστημα θεμάτων τα αναλαμβάνει αυτόματα

### 📁 Δομή Project

```
├── index.html       # Δομή σελίδας (χωρίς hardcoded προσωπικά δεδομένα)
├── script.js        # CONFIG, translations, όλη η λογική & animations
├── styles.css       # Θέματα, layout, responsive styles
